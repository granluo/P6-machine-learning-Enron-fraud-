...
def fraction_poi(data,numerator,denominator,new_feature):
    ...
    return data
data_dict = fraction_poi(data_dict,'from_poi_to_this_person','to_messages','ratio_from_poi') # Add new key, 'ratio_from_poi', into each people
data_dict = fraction_poi(data_dict,'from_this_person_to_poi','from_messages','ratio_to_poi') # Add new key, 'percentag_to_poi', into each people
features_list += ['ratio_from_poi','ratio_to_poi'] #  Add new features into the feature list for features split later
...
def feature_scaling(data_dictionary = data_dict, range_threshold = 10000000, f = features ):
    ...
    for i in range(len(feature_col)):
         features_range .append((features_list[i+1],max(feature_col[i])-min(feature_col[i])))
    print sorted(features_range, key = lambda x : x[1],reverse = True)


    scale_over_10m = [features_range[x] for x in range(len(features_range)) if features_range[x][1]> range_threshold]

    for person in data_dictionary:
        for large_scale in scale_over_10m:
            if data_dictionary[person][large_scale[0]] != 'NaN':
                data_dictionary[person][large_scale[0]]=float(data_dictionary[person][large_scale[0]])/large_scale[1]
...
pp.pprint(sorted_features_importance)
...
skb_feature_list = np.asarray(features_list[1:])[skb.get_support()]# get the list of k features selected by mutual_info_classif

pp.pprint (sorted(zip(features_list[1:],skb.scores_),key = lambda x: x[1],reverse = True))
features_list = ['poi']+list(set(dtc_feature_list+list(skb_feature_list)))
...
import pandas as pd
record = pd.DataFrame(columns = ['algorithm','accuracy','precision','recall','f1'])
...
scaled_features_train, scaled_features_test,features_train, features_test, labels_train, labels_test = \
    train_test_split(scaled_features, features, labels, test_size=0.3, random_state=42)
...
clf = RandomForestClassifier(n_estimators=5,criterion = "entropy",random_state = 42)
clf.fit(features_train,labels_train)
...
parameters = {'criterion':('entropy', 'gini'), 'max_depth':[2,3,4,5,6,7,8,9,10]}
dtc = DecisionTreeClassifier()
clf = GridSearchCV(dtc, parameters)
clf.fit(features, labels)

print '##############  Tuning The Decision Tree  ##############'
print pd.DataFrame(zip(clf.cv_results_['param_criterion'],clf.cv_results_['param_max_depth'],clf.cv_results_['rank_test_score'],clf.cv_results_['split0_test_score'],clf.cv_results_['split0_train_score'],clf.cv_results_['split1_test_score'],clf.cv_results_['split1_train_score']), columns = ['Criterion','Max depth','Rank of test score','split set 1 test score','split set 1 train score','split set 2 test score','split set 2 train score'])
...
record_tree = pd.DataFrame(columns = ['algorithm','accuracy','precision','recall','f1'])
set_num = 0
...
def kfold_sets_index(num_of_features,num_folds):
    ...
    for train_indices, test_indices in kf:
        ...
        sets_list.append((features_train_cv, features_test_cv, labels_train_cv, labels_test_cv))
    return sets_list



for j in (range(3,11,1)):
    clf = DecisionTreeClassifier(criterion="entropy",max_depth=j,random_state=43)
    prf_mean = []
    prf = []
    for i in kfold_sets_index(len(features_train),100):
        # print i
        ...
        prf.append(((accuracy_score(labels_test_cv,pred),)+precision_recall_fscore_support(pred, labels_test_cv)))
    for col in zip(*prf):
        prf_mean.append(np.mean(col,axis = 0))
    accuracy, precision, recall, f_score = prf_mean[0], prf_mean[1][1],prf_mean[2][1],prf_mean[3][1]
    record_tree.loc[len(record_tree)]= ['DT depth '+ str(j), accuracy, precision, recall, f_score]
